...
The column ImgUrl on table [dbo].[Products] must be changed from NULL to NOT NULL. If the table contains data, the ALTER script may not work. To avoid this issue, you must add values to this column for all rows or mark it as allowing NULL values, or enable the generation of smart-defaults as a deployment option.
*/
...
GO
ALTER TABLE [dbo].[Products] ALTER COLUMN [Description] TEXT NOT NULL;

ALTER TABLE [dbo].[Products] ALTER COLUMN [ImgUrl] VARCHAR (255) NOT NULL;
...
DECLARE tableconstraintnames CURSOR LOCAL FORWARD_ONLY
    FOR SELECT SCHEMA_NAME([schema_id]),
               ...
               0
        FROM   [sys].[objects]
        WHERE  [parent_object_id] IN (OBJECT_ID(N'dbo.Items'), OBJECT_ID(N'dbo.Categories'), OBJECT_ID(N'dbo.Messages'), OBJECT_ID(N'dbo.Tokens'), OBJECT_ID(N'dbo.UserConversations'), OBJECT_ID(N'dbo.UserRoles'))
               AND [type] IN (N'F', N'C')
